...
public class InjectResources {
    ...
    protected List<User> users = ResourcePool.readResource(ResourcePool.CREATE_USER_ENTITIES, new TypeReference<>() {
    });

    @BeforeEach
    void createsEnvironmentObjects() {
        items = ResourcePool.readResource(ResourcePool.CREATE_ITEM_ENTITIES, new TypeReference<>() {
        });
        users = ResourcePool.readResource(ResourcePool.CREATE_USER_ENTITIES, new TypeReference<>() {
        });
